...
const TravelApp: React.FC = () => {
  ...
  const handleStartChat = (message: string) => {
    ...
    // Show loader for a moment, then transition to chat
    setTimeout(() => {
      setShowLoader(false);
      setAppState('chat');
    }, 1500); // Extended for better UX
  };
  ...
  useEffect(() => {
    ...
    if (appState === 'chat' && initialMessage) {
      ...
      // Reset the initial message after it's been processed (wait longer than TravelChat's delay)
      setTimeout(() => {
        setInitialMessage('');
      }, 1000);
    }
  ...
  return (
    <div className="relative w-full h-screen overflow-hidden flex flex-col">
      ...
      <div className="relative flex-1 overflow-hidden">
        {/* Landing Page */}
        {(appState === 'landing' || (appState === 'transitioning' && showLoader)) && (
          <div 
            className={`absolute inset-0 z-10 ${
              appState === 'transitioning' && showLoader ? 'opacity-50' : 'animate-fade-in-up'
